...
import { initFts, rebuildFtsIndex } from './fts';
...
export async function clearPharmacyData(): Promise<void> {
...
export async function seedPharmacyData(): Promise<void> {
  ...
  for (const m of MEDICINES) {
    await run(
      `INSERT INTO medicines (name, description, category, supplier_id, price, stock, expiry_date)
       VALUES (?, ?, ?, ?, ?, ?, ?)`,
      [m.name, m.description, m.category, m.supplier_id, m.price, m.stock, m.expiry_date]
    );
  }
  ...
  await run(
    `INSERT INTO deliveries (supplier_id, medicine_id, quantity, delivery_date, cost)
     VALUES (?, ?, ?, ?, ?)`,
    [1, 1, 200, '2026-05-01', 1800]
  );
  await run(
    `INSERT INTO deliveries (supplier_id, medicine_id, quantity, delivery_date, cost)
     VALUES (?, ?, ?, ?, ?)`,
    [2, 2, 150, '2026-05-05', 950]
  );
