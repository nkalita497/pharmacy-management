...
router.get('/', (req: Request, res: Response): void => {
  const params = parsePagination(req, 'patients');
  let where = '1=1';
  ...
  db.get(`SELECT COUNT(*) as total FROM patients WHERE ${where}`, queryParams, (err, countRow: any) => {
    if (err) {
      res.status(500).json({ error: err.message });
      return;
    }
    db.all(
      `SELECT * FROM patients WHERE ${where} ORDER BY ${params.sort} ${params.order} LIMIT ? OFFSET ?`,
...
router.get('/:id', (req: Request, res: Response): void => {
  const { id } = req.params;
  db.get('SELECT * FROM patients WHERE id = ?', [id], (err, row: Patient) => {
    if (err) {
      res.status(500).json({ error: err.message });
      return;
    }
...
router.post('/', requireRole(['admin', 'pharmacist']), (req: Request, res: Response): void => {
  ...
  db.run(
    `INSERT INTO patients (first_name, last_name, pesel, email, phone) VALUES (?, ?, ?, ?, ?)`,
...
router.put('/:id', requireRole(['admin', 'pharmacist']), (req: Request, res: Response): void => {
  ...
  db.run(
    `UPDATE patients SET first_name=?, last_name=?, pesel=?, email=?, phone=? WHERE id=?`,
    [first_name, last_name, pesel, email, phone, id],
    function(err) {
      if (err) {
        res.status(500).json({ error: err.message });
        return;
